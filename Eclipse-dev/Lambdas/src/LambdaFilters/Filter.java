...
/*        Predicate<Person> ageFilter = p -> p.getAge() > 25;
        ...
        Predicate<Person> genderFilter = p -> p.getGender()
								        		.toLowerCase()
								        		.equals("female");
        
        System.out.println("Show female php programmers that earn more than 1400");
        phpProgrammers.stream()
        .filter(ageFilter)
        .filter(salaryFilter)
        .filter(genderFilter)
        .limit(1)
        .forEach(p -> System.out.printf("%s %s - salary: %s %n", p.getFirstName(),
        ...
        .filter(genderFilter)
        .limit(1)
        .forEach(p -> System.out.printf("%s %s - age: %s %n", p.getFirstName(),
